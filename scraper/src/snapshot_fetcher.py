...
def get_snapshots(url: str, log: bool = False, max_retries: int = 5, retry_delay: int = 120) -> list:
    """
    ...
    while attempts < max_retries:
        try:
            ...
            snapshots = _extract_snapshots(response.text)
            ...
            if log:
                logging.info(f"Found {len(snapshots)} snapshots for {university} {department}.")
...
def _extract_snapshots(response_text: str) -> list:
    """
